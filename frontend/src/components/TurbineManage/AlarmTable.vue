...
<el-table 
    :data="alarmItems.slice((currentPage - 1) * pageSize,currentPage * pageSize)" 
    ...
    <el-table-column 
        ...
        width="110"
        sortable
        :sort-method="(a,b)=>{return a.alarmLevel - b.alarmLevel}">

        <template #default="item">
            <el-tag :type = "alarmLevelColor[alarmLevelMeaning[item.row.alarmLevel]]">{{alarmLevelMeaning[item.row.alarmLevel]}}</el-tag>
        </template>
            ...
            <el-form :model='tmpItem' label-width='80px' :rules='formRule' ref='formRef'>
                ...
                <el-form-item label='报警级别' prop='alarmLevel'>
                    <el-select v-model = "tmpItem.alarmLevel" placeholder="请选择报警级别">
                        <el-option 
                            v-for = "(item,index) in alarmLevelMeaning" 
                            :key = 'index' 
                            :label = 'item' 
                            :value = 'index'>
                        </el-option>
                ...
                <el-form-item label='报警描述' prop='alarmDescription'>
                    <el-col :span="23">
                        <el-input 
                            v-model = "tmpItem.alarmDescription" 
                            placeholder="请填写报警相关描述"
                            clearable
                        />
                    </el-col>
                </el-form-item>
